...
struct PeerStreamActor {
    ...
    config: CommsConfig,                               // the active configuration for the stream
...
impl PeerStreamActor {
    ...
    async fn reader(tx: Sender<Arc<P2PMessage>>, mut reader: tokio::net::tcp::OwnedReadHalf, config: CommsConfig) {
